...
@section('title', 'Edit Media ' . $media->title . ' - Admin Flixora')
...
@section('content')
    ...
    <div style="margin-bottom: 2rem;">
        ...
        <h1 style="font-size: 1.8rem; font-weight: 800; color: #fff; margin-bottom: 0.3rem;">Edit Media: {{ $media->title }}</h1>
        <p style="color: var(--text-muted); font-size: 0.9rem;">Perbarui informasi detail untuk judul media ini.</p>
    </div>
        ...
        <form action="{{ route('admin.media.update', $media->id) }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem; flex-wrap: wrap;">
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" class="form-control" value="{{ old('title', $media->title) }}" required>
                </div>

                <div class="form-group">
                    <label for="type" class="form-label">Tipe Media *</label>
                    <select id="type" name="type" class="form-control" required>
                        <option value="movie" {{ old('type', $media->type) == 'movie' ? 'selected' : '' }}>Film (Movie)</option>
                        <option value="tv_show" {{ old('type', $media->type) == 'tv_show' ? 'selected' : '' }}>Acara TV (TV Show)</option>
                    </select>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div class="form-group">
                    ...
                    <input type="number" id="release_year" name="release_year" class="form-control" value="{{ old('release_year', $media->release_year) }}" required>
                </div>

                <div class="form-group">
                    <label for="duration_or_seasons" class="form-label">Durasi / Season *</label>
                    <input type="text" id="duration_or_seasons" name="duration_or_seasons" class="form-control" value="{{ old('duration_or_seasons', $media->duration_or_seasons) }}" required>
                </div>
            </div>
                    ...
                    @foreach($genres as $g)
                        <label style="display: flex; align-items: center; gap: 0.4rem; font-size: 0.85rem; color: #d1d5db; cursor: pointer;">
                            <input type="checkbox" name="genres[]" value="{{ $g->id }}" 
                                {{ in_array($g->id, old('genres', $selectedGenres)) ? 'checked' : '' }} 
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" class="form-control" required>{{ old('description', $media->description) }}</textarea>
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div class="form-group">
                    <label for="poster_url" class="form-label">URL Poster Gambar *</label>
                    <input type="url" id="poster_url" name="poster_url" class="form-control" value="{{ old('poster_url', $media->poster_url) }}" required>
                </div>

                <div class="form-group">
                    <label for="banner_url" class="form-label">URL Banner Backdrop (Opsional)</label>
                    <input type="url" id="banner_url" name="banner_url" class="form-control" value="{{ old('banner_url', $media->banner_url) }}">
                </div>
            </div>

            <div class="form-group">
                <label for="trailer_url" class="form-label">URL Embed Video Trailer YouTube (Opsional)</label>
                <input type="url" id="trailer_url" name="trailer_url" class="form-control" value="{{ old('trailer_url', $media->trailer_url) }}">
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div class="form-group">
                    <label for="director" class="form-label">Sutradara (Director)</label>
                    <input type="text" id="director" name="director" class="form-control" value="{{ old('director', $media->director) }}">
                </div>

                <div class="form-group">
                    <label for="cast" class="form-label">Pemeran Utama (Cast)</label>
                    <input type="text" id="cast" name="cast" class="form-control" value="{{ old('cast', $media->cast) }}">
                </div>
            </div>
